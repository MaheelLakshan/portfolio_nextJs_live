...
export const BentoGridItem = ({ className, id, title, description, img, imgClassName, titleClassName, spareImg }: { className?: string; id: number; title?: string | React.ReactNode; description?: string | React.ReactNode; img?: string; imgClassName?: string; titleClassName?: string; spareImg?: string }) => {
  const leftLists = ['NextJs', 'MERN-Stack', 'Cloud'];
  const rightLists = ['NestJs', 'Express', 'MySQL'];
  ...
  const handleCopy = () => {
    const text = 'maheellakshanra@gmail.com';
        ...
        <div className={cn(titleClassName, 'group-hover/bento:translate-x-2 transition duration-200 relative md:h-full min-h-40 flex flex-col px-5 p-5 lg:p-10')}>
            ...
            <div className="flex gap-1 lg:gap-5 w-fit absolute -right-3 lg:-right-2 h-full">
              ...
              <div className="flex flex-col gap-3 md:gap-3 lg:gap-4">
                ...
                {rightLists.map((item, i) => (
                  <span
                    key={i}
                    className="lg:py-3 lg:px-6 py-2 px-3 text-xs lg:text-base opacity-50 
                    lg:opacity-100 rounded-lg text-center bg-[#10132E]"
                  >
                    {item}
                  </span>
          ...
          {id === 6 && (
            <div className="mt-5 relative">
              ...
              <MagicButton title={copied ? 'Email is Copied!' : `Copy my Email`} icon={<IoCopyOutline />} position="right" handleClick={handleCopy} otherClasses="!bg-[#161A31]" />
            </div>
          )}
        </div>
      </div>
